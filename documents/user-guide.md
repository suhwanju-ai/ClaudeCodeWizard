...
템플릿의 원시 JSON을 바로 확인할 수 있다. **저장**은 디스크에만 반영하고,
**실행**은 저장 후 바로 폴더 선택 다이얼로그를 띄워 파이프라인을 시작한다.

### 4.3 파이프라인 실행 화면

좌측에 단계 타임라인(대기/진행/체크포인트 대기/승인됨/실패 상태를 점 색으로 표시),
우측에 실시간 로그(어시스턴트 텍스트, 도구 사용, 도구 결과, 최종 결과, 에러)가
`pipeline://stage-event`를 통해 실시간으로 쌓인다. 체크포인트 단계에 도달하면
하단에 다음 3개 버튼이 나타난다:

| 버튼 | 동작 |
|---|---|
| **승인** | 현재 단계를 완료 처리하고 다음 단계로 진행(마지막 단계면 파이프라인 완료) |
| **수정 요청 보내기** | 입력한 피드백 텍스트로 **같은 단계를 재실행**(원래 프롬프트를 완전히 대체) |
| **거부** | 프로세스를 다시 실행하지 않고 파이프라인 자체를 취소(cancelled) 처리 |

변경된 파일 배지(Write/Edit 도구 호출에서 감지한 경로)가 체크포인트 카드 위에
같이 표시되어, 승인 전에 무엇이 바뀌었는지 훑어볼 수 있다.
...
- **새로 만든 폴더만 지원** — 템플릿은 사용자가 고른 폴더에 대해 실행되며, 이미
  파이프라인 실행 기록이 있는 폴더에 대한 재실행/중복 방지는 지원하지 않는다.
